...
def test_matMul ( n, path="./", verbose=False ):
    ...
    try:
        ...
        if verbose:
            ...
            print ("result")
            print (resultlist)
        assert resultlist == answer, "The matrix multiplication result doesn't match answers/matrix_c_{}x{}.txt.".format(n,n)
        return True
...
def grade_cacheBlocking( path='./', verbose=False ):
    ...
    for i in range(2,6):
        if test_cacheBlocking(2**i,"trace_matMul_{}x{}".format(2**i,2**i),path,verbose):
            score += 6
        else:
            break

    print ("Score on cacheBlocking: {} out of 24.".format(score))
    return score
