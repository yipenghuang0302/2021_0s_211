...
def generate_test ( n, path="./" ):
    ...
    matrix_b = [ [random.randrange(8) for j in range(n)] for i in range(n) ]
...
def test_matMul ( n, path="./", verbose=False ):
    ...
    try:
        ...
        # print ("resultlist")
        # print (resultlist)
        assert resultlist == answer, "The matrix multiplication result doesn't match answers/matrix_c_{}x{}.txt.".format(n)
        return True
...
def grade_matMul( path='./', verbose=False ):
    ...
    for i in range(6):
        if test_matMul(2**i,path,verbose):
            score += 4
        else:
            break

    print ("Score on matMul: {} out of 24.".format(score))
    return score
